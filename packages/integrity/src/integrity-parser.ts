...
import { _splitPath } from './integrity-calculator'
...
export function extractIntegrity(target: object): IntegrityCheckResult[] {
  ...
  return Object.entries(target).flatMap(([key, val]) => {
    ...
    if (!val || typeof val !== 'string') {
      return err(`sd-jwt integrity '${key}' should be string 'alg-hash?opt': ${JSON.stringify(val, undefined, 2)}`)
    }
    ...
    for (const step of path) {
      if (typeof value === 'object' && step in value) {
        ...
        return err(`sd-jwt integrity for '${fullPath}' has no value in ${JSON.stringify(target, undefined, 2)}`)
      }
    }
    return [
      {
