...
describe('Integrity Type Edge Cases', () => {
  ...
  test('handles bracket notation for array indices', () => {
  ...
  test('handles complex URI keys at the root', () => {
    type Result = Integrity<Credentials, "['https://example.com/vocab'].description">

    // No dot before the first bracket
    expectTypeOf<Result>().toHaveProperty("['https://example.com/vocab'].description#integrity")
  })
