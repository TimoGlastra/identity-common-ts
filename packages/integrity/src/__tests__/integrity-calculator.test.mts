...
import { calculateIntegrity } from '../integrity-calculator'
...
describe('Integrity Calculator', () => {
  test('1. Root Level Simple Keys (Dot Notation)', async () => {
    const result = await calculateIntegrity(mockCredentials, ['vct', 'iss'], mockHasher)

    expect(result).toHaveProperty('vct#integrity')
    expect(result['vct#integrity']).toBe('sha256-vct#integrity?https://example.com/vct')

    expect(result).toHaveProperty('iss#integrity')
    expect(result['iss#integrity']).toBe('sha256-iss#integrity?https://example.com/iss')
  })

  test('2. Deep Nesting (Dot Notation)', async () => {
    const result = await calculateIntegrity(mockCredentials, ['evidence.document.id'], mockHasher)
  ...
  test('4. Array Indices (Explicit)', async () => {
    const result = await calculateIntegrity(mockCredentials, ['arr[0]', 'arr[1]'], mockHasher)
  ...
  test('5. Wildcard Expansion (*) on Objects (Normalization Check)', async () => {
    ...
    const result = await calculateIntegrity(mockCredentials, ['bank_accounts.*.iban'], mockHasher)
    ...
    expect(result).toHaveProperty("bank_accounts['complex-bank'].iban#integrity")
    expect(result["bank_accounts['complex-bank'].iban#integrity"]).toContain('EU456')
  })

  test('6. Wildcard Expansion [*] on Arrays', async () => {
    const result = await calculateIntegrity(mockCredentials, ['arr[*]'], mockHasher)
  ...
  test('8. Complex Root Keys (URI keys)', async () => {
    const result = await calculateIntegrity(mockCredentials, ["['https://example.com/vocab'].description"], mockHasher)

    expect(result).toHaveProperty("['https://example.com/vocab'].description#integrity")
    expect(result["['https://example.com/vocab'].description#integrity"]).toContain('complex vocabulary')
  })
  ...
  test('10. Wildcard on Non-Object/Null (Safety Check)', async () => {
    ...
    const result = await calculateIntegrity(payload, ['primitive.*', 'nullVal.*'], mockHasher)
